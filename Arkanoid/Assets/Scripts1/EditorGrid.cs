...
{
    private const float _leftPosition=-7.48f;
    //private const float _rightPosition = 7.7f;
    private const float _upPosition=4.2f;
    private const int _columnCount = 16;
    private const int _lineCount = 14;
    ...
    public Vector3 CheckPosition(Vector3 position) 
    {
        ...
        if (position.x > x && position.x < (x + _offsetRight * _columnCount) && position.y < y && position.y > (y - _offsetDown * _lineCount))
        { 
